    ...
    function showTable(selectedDate) {
        ...
        horaires.forEach((time) => {
            ...
            tr.innerHTML = `
                <td>${time}</td>
                <td>
                    <input type="checkbox" id="slot-checkbox-${time}" ${slotStatus === 'Réservé' ? 'checked' : ''} ${slotStatus === 'Bloqué' ? 'checked' : ''} disabled> 
            ...
            if (slotStatus === 'Bloqué') {
                tr.style.backgroundColor = "rgba(213, 213, 213, 0.64)"; // Gris clair
            }
